...
assert.deepEqual(
  githubActivity.configuredRepositoryLabels({ repositories: ['example/PublicRepo', 'example/PrivateRepo', 'example/PrivateRepo'] }),
...
const recentWithDraft = githubActivity.recentActivityItems(
  {
    issues: [{ key: 'TEST-13', summary: 'Dashboard work', updatedAt: '2026-08-15T11:58:00Z', url: 'https://example.test/TEST-13' }]
  },
  {
    pullRequests: [
      { repository: 'example/PrivateRepo', number: 4, title: 'Draft work', updatedAt: '2026-08-15T11:59:00Z', url: 'https://github.com/example/PrivateRepo/pull/4', state: 'DRAFT' },
      { repository: 'example/PublicRepo', number: 133, title: 'Ready work', updatedAt: '2026-08-15T11:57:00Z', url: 'https://github.com/example/PublicRepo/pull/133', state: 'OPEN' }
    ]
  }
);
assert.equal(recentWithDraft[0].identity, 'PrivateRepo #4', 'a genuinely recent draft should participate in recent activity ordering');
...
const crowdedJira = Array.from({ length: 12 }, (_, index) => ({
  ...
  url: `https://example.test/TEST-${index + 20}`
}));
const balancedRecent = githubActivity.recentActivityItems(
  { issues: crowdedJira },
  { pullRequests: [{ repository: 'example/PrivateRepo', number: 5, title: 'Draft work', updatedAt: new Date(NOW - 60_000).toISOString(), url: 'https://github.com/example/PrivateRepo/pull/5', state: 'DRAFT' }] }
);
